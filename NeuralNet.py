...
class NeuralNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        inputNeurons, hiddenNeurons, outputNeurons = 784, 500, 22
...
def SetupData(digits, letters, ground_truths):
    ...
    final_truths = torch.zeros(digits.shape[0] + letters.shape[0], 22)
...
# Oversamples the data to strengthen the amount of data for the minority class
def OversampleData(letters, labels):
    ...
    while i < len(labels) and not(isFull):
        ...
        if labels[i] in accepted_letters:

            for k in range(2):

                new_dataset[count + k] = letters[i]
                new_labels[count + k] = accepted_letters.index(labels[i]) + 10
...
if __name__ == "__main__":
    ...
    print("Shape of numbers: ", x_train.shape)
    ...
    print("Shape of letter: ", letters.shape)
    labels = torch.from_numpy(labels) + 9 # Add 9 to differentiate from 0-9 digits

    # Call the oversample method to oversample the data
    new_letters, new_labels = OversampleData(letters, labels)

    ground_truths = torch.cat([y_train, new_labels])
    print(ground_truths.shape)

    # Create the dataloader that will contain all the data points shuffled through the SetupData function
    imageDataLoader, groundTruthsDataLoader = SetupData(x_train, new_letters, ground_truths)
    ...
    # Run second test case through the neural network
    ...
    result = neuralNet.Test(test_letters_flat[3])
    for i in range(22):
